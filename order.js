document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        const response = await fetch("https://you-fashion-backend.vercel.app/api/orders/", {
            method: "GET",
            headers: {
        ...
        orders.forEach(order => {
            ...
            let itemsHTML = order.items.map(item => `
                <div class="flex items-center space-x-4 border-b pb-4">
                    <img src="https://you-fashion-backend.vercel.app${item.food_item.image}" alt="${item.food_item.name}" class="w-16 h-16 object-cover rounded">
                    <div>
